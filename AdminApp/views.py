        ...
        if key=='vsinadmin':
            for i in range(0, len(category_name)):
                ...
                if catobj:
                    ...
                    Adminobj.trending_category_name = category_name[i].get('catname')
                    Adminobj.trending_category_id = category_name[i].get('id')
                    Adminobj.save()
                else:
                    print("not found")
                    if catobjname:
                        Adminobjprio = TrendingCategories.objects.get(trending_category_name=catobjname[0].get('trending_category_name'))
                        Adminobjprio.trending_priority = category_name[i].get('priority')
                        Adminobjprio.trending_category_id = category_name[i].get('id')
                        Adminobjprio.save()
                    else:
        ...
        if key == 'vsinadmin':
            for i in range(0, len(sub_category_data)):
                ...
                if catobj:
                    ...
                    print("not found")
                    if catobjname:
                        Adminobjprio = TrendingSubCategories.objects.get(trending_sub_category_name=catobjname[0].get('trending_sub_category_name'))
                        Adminobjprio.trending_sub_categories_priority = sub_category_data[i].get('priority')
...
def fetch_admin_trending_categories(request):
    ...
    try:
        if key=='vsinadmin':
            trendingcategoryobj1=TrendingCategories.objects.filter().values()
            if len(trendingcategoryobj1)>0:
                for i in range(0,len(trendingcategoryobj1)):
                    arrayval.append(int(trendingcategoryobj1[i].get('trending_priority')))
                val = sorted(arrayval)
                if len(val) > 0:
                    for i in range(0, len(val)):
                        trendingcategoryobj = TrendingCategories.objects.filter(trending_priority=val[i]).values()
            ...
            if len(selectedsubcategoryobj)>0:
                ...
                if len(sortvalue)>0:
                    for i in range(0,len(sortvalue)):
                        selectedsubcategoryobj = AdminSelectedSubCategories.objects.filter(sub_categories_priority=sortvalue[i]).values()
