            ...
            if len(bidobj)>-0:
                for i in range(0,len(bidobj)):
                    ...
                    if converted_deadline > date.today():
                        ...
                        basicobj=BasicCompanyDetails.objects.filter(updated_by_id=bidobj[i].get('updated_by_id')).values()
                        source_create_list.append({'source_id':bidobj[i].get('source_id'),
...
def vendor_source_responses(request):
    ...
    try:
        ...
        if len(sourceobj)>0:
            ...
            return Response({'status':200,'message':'Vendor Response for Source','data':sourcepublishobj,'pf_charges':sourceobj[0].get('p_f_charges'),'frieght_charges':sourceobj[0].get('frieght_charges'),'delivery':sourceobj[0].get('delivery'),'warranty':sourceobj[0].get('warranty'),'payment':sourceobj[0].get('payment_terms')},status=status.HTTP_200_OK)
        else:
                ...
                if basicval:
                    ...
                    if billingobj:
                        ...
                        if converted_deadline < date.today():
                            ...
                            print('Not Expired',sourcobj[i].get('deadline_date'))
                    else:
                        listarray.append({'id': sourcobj[i].get('id'),
                                          'company_code': basicval[0].get('company_code'),
                                          'company_name': basicval[0].get('company_name'),
